...
### Мобайл (<768px) — аккордеон в бургере с разведением зон
- Внутри `.mobile-nav__menu` пункт Portfolio — раскрывающийся аккордеон со списком кейсов («All cases →» в конце), стиль мобильных пунктов (tap-target ≥44px). Полностью CSS-only.
- **Разведение тапа (по запросу пользователя):** строка Portfolio делится на две зоны —
  - тап по **слову «Portfolio»** (`<a class="mobile-nav__sublink">`) → переход на `/portfolio.html`;
  - тап по **шеврону / правой части строки** (`<label class="mobile-nav__subchevron">`) → раскрытие списка.
- **Механика — валидный checkbox-hack (НЕ `<details>`):** изначально планировался вложенный `<details>`, но `<a>` внутри `<summary>` — невалидный HTML (interactive-в-interactive) и ненадёжен в Safari/Firefox. Поэтому: визуально скрытый, но фокусируемый `<input type="checkbox" id="m-portfolio-toggle">` + `<label for>` управляют раскрытием через `.mobile-nav__subtoggle:checked ~ .mobile-nav__sublist { display:flex }`. `<a>` и `<label>` — siblings в обычном `<div>`, без вложенного interactive. Внешний бургер остаётся настоящим `<details class="mobile-nav">`.
- Доступность: чекбокс фокусируем с клавиатуры (Space раскрывает), `aria-label="Show Portfolio cases"`.
...
3. **Мобайл 390:** бургер → тап по слову «Portfolio» ведёт на `/portfolio.html`; тап по шеврону/правой зоне раскрывает аккордеон со списком; layout не ломается; горизонтального скролла нет.
